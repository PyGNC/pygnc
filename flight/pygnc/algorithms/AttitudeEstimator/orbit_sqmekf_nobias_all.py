...
import math
from .sqmekf_core_nobias_all import SQMEKFCore_nb_all
...
def process_dynamics(x, u, h):
    ...
    theta = np.linalg.norm(omega)*h

    r = (omega)/np.linalg.norm(omega)
    ...
    x_next = qnext/np.linalg.norm(qnext)

    return x_next

class OrbitSQMEKF_nb_all(SQMEKFCore_nb_all):
    """
    ...
    def __init__(self):
        ...
        def measurement_function(x, inertial_measurement):
            ...
            #just estimating the quaternion
            C = np.block([[2*hat(sun_predicted_n)], [2*hat(mag_predicted_n)]])
        ...
        Q_noise = np.identity(3)*(term1)

        #print("THIS IS Q NOISE: ", Q_noise)

        #Q_noise = np.eye(3)*1e-5
...
#the problem is that the predicted sun vector isn't being equal to the actual sun vector