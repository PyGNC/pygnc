...
#Need to also estimate out a magnetometer bias
...
class OrbitMEKF(MEKFCore):
    ...
    def __init__(self):
        ...
        def measurement_function(x, inertial_measurement):
            ...
            measurement = Q_m @ inertial_measurement

            C = np.block([[2*hat(measurement[0:3]), np.zeros((3,3))], [2*hat(measurement[3:6]), np.zeros((3,3))]])
        ...
        Q_noise = block_diag(np.identity(3)*(0.1*math.pi/180)**2, np.identity(3)*(0.001*math.pi/180)**2)
